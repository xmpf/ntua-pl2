...
mkConstraints :: Expr -> Env -> Int -> Maybe (Type, Constr, Int)
mkConstraints (Evar str) env var = do
    lookup str env >>= \t -> Just (Tvar t, [], var)
mkConstraints (Eabs str expr) env var = do
    mkConstraints expr ((str, var + 1) : env) (var + 1) >>=
      \(a, b, c) -> Just ((Tfun (Tvar (var + 1)) a), b, c + 1)
...
tReplace :: Type -> Type -> Constr -> Constr -> Constr
tReplace _ _ [] c' = c'
tReplace ta tb ((ta', tb') : c) c' = tReplace ta tb c ((ta'', tb'') : c')
  where ta'' = replace ta tb ta'
        tb'' = replace ta tb tb'
        replace a b c@(Tvar d)
          | a == c = b
          | otherwise = c
        replace a b c@(Tfun e' e'') = Tfun (replace a b e') (replace a b e'')

-- unify W algorithm
unify :: Constr -> Rules -> Maybe Rules
unify [] r = Just r
unify ((ta, tb) : tc) r
  | (ta == tb) = unify tc r
unify ((ta@(Tvar a), tb) : tc) r
  | not (tLookup ta tb) = unify (tReplace ta tb tc []) ((ta, tb) : r)
unify ((ta, tb@(Tvar b)) : tc) r 
  | not (tLookup tb ta) = unify (tReplace tb ta tc []) ((tb, ta) : r)
unify ((ta@(Tfun ta' ta''), tb@(Tfun tb' tb'')) : tc) r = unify ((ta', tb') : (ta'', tb'') : tc) r
unify _ _ = Nothing

sub1 :: Type -> Rules -> Type
sub1 ta@(Tvar a) r =
    case lookup ta r of
...
sub' :: Type -> Rules -> Maybe Type
sub' t r = do
    x <- pure $ sub1 t r 
    if t == x then Just t else sub' x r
