...
'''
Στο συγκεκριμένο πρόβλημα θα μπορούσα να χρησιμοποιήσω έναν generator για τη
δημιουργια συμβολοσειρών.
Ωστόσο στην εκφώνηση μας δίνεται η δυνατότητα για hardcoded προβλήματα.

Επίσης "έκλεψα" τον HTML και CSS απο την ιστοσελίδα του μαθήματος για να μη γράφω δικά μου.
'''
...
def index():
    ...
    return  '''<!DOCTYPE html PUBLIC
      ...
      <form action="/palseq/" id="f" name="f" method="post">''' + str(message) + '''<table border="0" cellspacing="3">
      <tr>
        <td><input type="text" class="box" name="answer" id="answer" autofocus /></td>
        <td><input type="submit" class="button" name="submit" id="submit" value="''' + str(label) + '''"/></td>
      </tr>
      </table>
      </form>
